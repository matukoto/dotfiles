...
- 歴史的な理由で `dot_foo` や `dot_config/...` というパス名は残っていたが、
  実際の配備は repo ルートの flake と `config/home-manager/modules/*.nix` が担当する。
...
- 単一対象の確認例:
  ...
  - `stylua --check config/nvim/lua/plugins/lspconfig.lua`
  - `typos README.md`
...
- このリポジトリは大きく 4 層で構成されている。
  ...
  3. **ツール管理レイヤー**:
     `config/aqua/aqua.yaml` は CLI ツール本体、
     `config/mise/config.toml` は言語ランタイムと npm 系 CLI、
     `config/home-manager/home.nix` は
      Home Manager module を束ねてパッケージや dotfile 配備を管理する。
  4. **アプリ設定レイヤー**:
     `config/nvim`、`config/home-manager/fish`、
     `claude`、`copilot`、`local` などが実設定を持つ。
- repo ルートの `flake.nix` が `darwin` / `linux` / host-specific Linux の出力を作り、
  `home.nix` に共通パッケージ、
  `darwin.nix` / `linux.nix` / `modules/*.nix` に役割別の差分を置く。
- Fish は `config/home-manager/fish/config.fish` をソースとして持つ。
  `config/home-manager/modules/fish.nix` が
  OS ごとの行と wrapper 関数を差し込んだうえで
  `programs.fish.shellInit` と `xdg.configFile` に配備する。
  生成後の `~/.config/fish/config.fish` ではなく、
  リポジトリ側のソースを修正する。
- Neovim は `config/nvim` 以下の Lua 設定が中心で、
...
- 新しい CLI ツールやバージョン更新は追加先を分ける。
  - 汎用 CLI: `config/aqua/aqua.yaml`（checksum 必須）
  - 言語ランタイム / npm ベース CLI: `config/mise/config.toml`
  - エディタ依存の LSP / formatter / linter: `config/home-manager/home.nix`
  - dotfile / config file 配備: `config/home-manager/modules/*.nix`
- GitHub Copilot CLI の起動ポリシーは shell wrapper で管理する。
  fish は `config/home-manager/fish/functions/`、
  bash は `bash_aliases` を確認する。
  `trusted_folders` のような machine-specific な永続設定は
  リポジトリではなくローカルの `~/.copilot/config.json` を使う。
- Fish 関連を変えるときは
  `config/home-manager/fish/` と
  `config/home-manager/modules/fish.nix` を合わせて確認する。
  Nix 側で OS 差分と生成ハッシュを入れている。
- Neovim の LSP は Mason ではなく Home Manager / Nix 管理。
  LSP を増減するときは `config/home-manager/home.nix` と
  `config/nvim/lua/plugins/lspconfig.lua` の両方を見る。
- 永続的な設計メモや調査メモの追加を依頼された場合は、
  既存の `config/opencode/AGENTS.md` に合わせて
