...
let
  ...
  switchCommand =
    if pkgs.stdenv.isDarwin then
      ''
      def ca [] {
        ^sudo -H nix --extra-experimental-features "nix-command flakes" run $"($env.HOME)/.local/share/chezmoi/dot_config/home-manager#darwin-rebuild" -- switch --flake $"($env.HOME)/.local/share/chezmoi/dot_config/home-manager#${hostname}"
      } # 現在のホスト向け設定を適用
      ''
    else
      ''
      def ca [] {
        ^nix --extra-experimental-features "nix-command flakes" run $"($env.HOME)/.local/share/chezmoi/dot_config/home-manager#home-manager" -- switch --flake $"($env.HOME)/.local/share/chezmoi/dot_config/home-manager#${hostname}"
      } # 現在のホスト向け設定を適用
      '';
