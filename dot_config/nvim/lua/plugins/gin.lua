...
return {
  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd('FileType', {
      pattern = 'gin-log',
      group = gin_augroup,
      callback = function() vim.schedule(_G.MyGinLogSettings) end, -- Schedule to ensure buffer is ready
    })
    vim.api.nvim_create_autocmd('FileType', {
      pattern = 'gin-status',
      group = gin_augroup,
      callback = function() vim.schedule(_G.MyGinStatusSettings) end,
    })
    vim.api.nvim_create_autocmd('BufRead', {
      pattern = 'ginedit://*',
      group = gin_augroup,
      callback = function() vim.schedule(_G.MyGinPatchSettings) end,
    })
