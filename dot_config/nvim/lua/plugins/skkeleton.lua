...
return {
  ...
  -- Load when entering insert mode or command-line mode, or specific events
  ...
  config = function()
    ...
    if vim.fn.exists('*skkeleton#config') ~= 2 or vim.fn.exists('*skkeleton#register_kanatable') ~= 2 then
    ...
    vim.keymap.set({'i', 'c'}, '<C-j>', '<Plug>(skkeleton-enable)', { noremap = false, desc = "Enable Skkeleton" })
  end,
}
