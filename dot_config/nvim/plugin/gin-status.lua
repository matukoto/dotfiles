...
preview.winid_autocmds = {}
function preview.dispose(winid)
  local autocmds = preview.winid_autocmds[winid]
  ...
  preview.winid_autocmds[winid] = nil
end
...
local function get_diff_bufnr(status_winid)
  ...
  local diff_bufname = ('gindiff://%s;#[%%22%s%%22]'):format(
    vim.fn['gin#util#worktree'](),
    diff_file_relative_path
  )
  local diff_bufnr
...
function preview.open(status_winid)
  ...
  local close_auid = vim.api.nvim_create_autocmd({ 'BufWinLeave', 'BufHidden', 'BufUnload' }, {
    ...
    callback = function()
      if not vim.api.nvim_win_is_valid(status_winid) then
        preview.dispose(status_winid)
        return
      end
      preview.close(status_winid)
    end,
  })
  ...
  local resize_auid = vim.api.nvim_create_autocmd('WinResized', {
    group = augroup,
    callback = function()
      preview.resize(status_winid)
    end,
  })
  preview.winid_autocmds[status_winid] = { close_auid, redraw_auid, resize_auid }
end

function preview.close(status_winid)
  local preview_winid = vim.w[status_winid].gin_status_preview_winid
  if preview_winid == nil then
    return
  end
...
function preview.resize(status_winid)
  if not vim.api.nvim_win_is_valid(status_winid) then
    preview.dispose(status_winid)
    return
  end
...
vim.api.nvim_create_autocmd('BufWinEnter', {
  ...
  callback = vim.schedule_wrap(function(ctx)
    preview.open(vim.fn.bufwinid(ctx.buf))
  end),
})
